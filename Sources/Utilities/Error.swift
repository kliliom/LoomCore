...
public enum LoomError: Error, Equatable {
  /// `SQLITE_ERROR`
  ///
  ...
  /// `SQLITE_RANGE`
  ///
  /// Parameter number argument to one of the `sqlite3_bind` or the `sqlite3_column` routines is out of range.
  ...
  /// Initializes an `LoomError` from an SQLite error code.
  /// - Parameters:
